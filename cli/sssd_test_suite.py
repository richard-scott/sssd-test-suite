...
class BasicVagrantCommand(Command):
    def __init__(self, subparser, name, help, command=None):
        ...
        self.parser.add_argument(
            ...
            help='Run command on all machines (default)'
        )

        self.parser.add_argument(
            '--ad', action=UniqueAppendConstAction, dest='guests', const='ad',
            help='Run command on AD machine'
        )

        self.parser.add_argument(
            '--ad-child', action=UniqueAppendConstAction, dest='guests', const='ad-child',
            help='Run command on AD child machine'
        )

        self.parser.add_argument(
            '--ipa', action=UniqueAppendConstAction, dest='guests', const='ipa',
            help='Run command on IPA machine'
        )

        self.parser.add_argument(
            '--ldap', action=UniqueAppendConstAction, dest='guests', const='ldap',
            help='Run command on LDAP machine'
        )

        self.parser.add_argument(
            '--client', action='append_const', dest='guests', const='client',
            help='Run command on client machine'
        )
...
class ProvisionHostCommand(Command):
    def __init__(self, subparser, name, help):
        ...
        self.parser.add_argument(
            ...
            help='Location of libvirt storage pool that will be named as "sssd-test-suite"',
            required=True
        )
...
class ProvisionGuestsCommand(Command):
    def __init__(self, subparser, name, help):
        ...
        self.parser.add_argument(
            '--ad', action=UniqueAppendConstAction, dest='guests', const='ad',
            help='Provision AD machine'
        )

        self.parser.add_argument(
            '--ad-child', action=UniqueAppendConstAction, dest='guests', const='ad-child',
            help='Provision AD child machine'
        )

        self.parser.add_argument(
            '--ipa', action=UniqueAppendConstAction, dest='guests', const='ipa',
            help='Provision IPA machine'
        )

        self.parser.add_argument(
            '--ldap', action=UniqueAppendConstAction, dest='guests', const='ldap',
            help='Provision LDAP machine'
        )

        self.parser.add_argument(
            '--client', action='append_const', dest='guests', const='client',
            help='Provision client machine'
        )
...
class PruneBoxCommand(Command):
    ...
    def run(self, args, params=[]):
        result = self.vagrant.run(['box', 'prune'], params, stdout=subprocess.PIPE)

        for (box, version) in self.re.findall(result.stdout.decode('utf-8')):
            imgfile = '%s_vagrant_box_image_%s.img' % (box.replace('/', '-VAGRANTSLASH-'), version)
...
class CreateBoxCommand(Command):
    ...
    class Box:
        def __init__(self, createbox, guest, args):
            ...
            if guest in createbox.linux:
                self.vgfile =  '%s/boxes/vagrant-files/linux.vagrantfile' % createbox.dir
            else:
                self.vgfile =  '%s/boxes/vagrant-files/windows.vagrantfile' % createbox.dir
    ...
    def __init__(self, subparser, name, help):
        ...
        self.parser.add_argument(
            ...
            help='Output directory where new boxes will be stored (default = %s/boxes)' % self.dir,
        ...
        self.parser.add_argument(
            '--ldap', action=UniqueAppendConstAction, dest='guests', const='ldap',
            help='Create LDAP box'
        )

        self.parser.add_argument(
            '--client', action='append_const', dest='guests', const='client',
            help='Create client box'
        )
        ...
        self.tasks = [
            self.Task('Make all images readable', self.guests, self.taskMakeReadable),
            self.Task('Halting all guests', [], self.taskHalt),
            self.Task('Zero out empty space on linux machines', self.linux, self.taskZeroDisk),
            self.Task('Create boxes', self.guests, self.taskCreateBox),
            self.Task('Create metadata', self.guests, self.taskCreateMetadata)
        ]

    def run(self, args, params=[]):
        print('This operation may take hours to finish. Be patient.')
        print('It may ask you a sudo password for command: chmod a+r %s/*.' % self.pool)
    ...
    def taskProvision(self, args):
        limit = ['all'] if args.guests is None else args.guests
        self.ansible.run('prepare-guests.yml', limit, ['--skip-tags=enroll-all'])
    ...
    def taskZeroDisk(self, guest, box, args):
        ...
        self.shell.run(['qemu-img', 'convert', '-O', 'qcow2', box.backup, box.img])
...
def main():
    ...
    parser.add_argument(
        ...
        help='Do not perform any chanes. Only print commands that would be executed.'
    )
